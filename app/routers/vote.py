...
def create_vote(vote:schemas.VoteBase,db:Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if(vote.dir == 1):
        ...
        db.commit()
        db.refresh(new_vote)
        return {"message": "successfulyy added vote"}
    else:
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Vote does not exist")
        vote_query.delete(synchronize_session=False)
        db.commit()
        return {"message": "successfullt deleted vote"}
